					  ...
					  enum frame_encryption encrypted)
{
	...
	wpa_msg(sm->ctx->msg_ctx, MSG_INFO, "M1_KEY_INFO %d", key_info);
#endif /* CONFIG_WFA */
...
#ifdef CONFIG_WFA
	wpa_msg(sm->ctx->msg_ctx, MSG_INFO, "EAPOL-M1-ML-VALIDATE-SUCCESS "
		"addr=" MACSTR,
		MAC2STR(sm->bssid));
#endif
...
#ifdef CONFIG_WFA
	size_t hex_len = 2 * key_data_len + 1;
	char *hex = os_malloc(hex_len);
	if (hex) {
		wpa_snprintf_hex(hex, hex_len, key_data, key_data_len);
		wpa_msg(sm->ctx->msg_ctx, MSG_INFO, "M3_KDE_IE %s", hex);
		os_free(hex);
	}
#endif /* CONFIG_WFA */
					  ...
					  size_t key_data_len)
{
	...
	wpa_msg(sm->ctx->msg_ctx, MSG_INFO, "M3_KEY_INFO %d", key_info);
#endif /* CONFIG_WFA */
...
#ifdef CONFIG_WFA
	wpa_msg(sm->ctx->msg_ctx, MSG_INFO, "EAPOL-M3-ML-VALIDATE-SUCCESS "
		"addr=" MACSTR,
		MAC2STR(sm->bssid));
#endif
...
#endif /* CONFIG_TESTING_OPTIONS */
#ifdef CONFIG_WFA
	case WPA_PARAM_RANDOM_PMKID:
		sm->random_pmkid = value;
		break;
#endif /* CONFIG_WFA */
#ifdef CONFIG_DPP2
