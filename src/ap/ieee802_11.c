...
#ifdef CONFIG_WFA
    if (hapd->iconf->rrm_res_bits) {
        /* rrm: res b36-39 */
        *(eid + RRM_CAPABILITIES_IE_LEN - 1) |= 0xF0;
        wpa_printf(MSG_DEBUG, "eid(%d): Set reserved bits",WLAN_EID_RRM_ENABLED_CAPABILITIES);
    }
#endif
...
#ifdef CONFIG_WFA
	/* reserved bits: 2, 3, 14, 15 */
	if (hapd->iconf->capa_info_res_bits) {
		capab |= 0xC00C;
	}
#endif
...
/* WFA add hapd argu for extended bytes config */
...
#ifdef CONFIG_WFA
	if (hapd->iconf->tpe_extend) {
		eid = hostapd_wfa_element_extend(eid, len_pos);
	}
#endif
...
u8 * hostapd_eid_txpower_envelope(struct hostapd_data *hapd, u8 *eid)
{
	...
	if (is_6ghz_op_class(iconf->op_class)) {
		...
		eid = hostapd_add_tpe_info(eid, tx_pwr_count, tx_pwr_intrpn,
					   REG_DEFAULT_CLIENT, tx_pwr, hapd);
		...
		if (iconf->reg_def_cli_eirp != -1 &&
			...
			eid = hostapd_add_tpe_info(
				...
				REG_DEFAULT_CLIENT,
				hapd->iconf->reg_def_cli_eirp, hapd);
...
#ifdef CONFIG_WFA
	if (hapd->iconf->wide_band_cs_extend) {
		eid = hostapd_wfa_element_extend(eid, len_pos);
	}
#endif
...
#ifdef CONFIG_WFA
	/* There is no RNR if eid = eid_start */
	if (hapd->iconf->rnr_extend && eid > eid_start) {
		/* Add one reserved byte in TBTT infor for non MLD */
		if (*(eid_start + 1) < 20) {
			*(eid_start + 1) += 1;
			*(eid_start + 3) += 1; /* TBTT Length */
			*eid++ = 0xff;

		}
		eid = hostapd_wfa_element_extend(eid, eid_start + 1);
	}
#endif
...
#ifdef CONFIG_WFA
	if (hapd->iconf->mbssid_extend) {
		eid = hostapd_wfa_element_extend(eid, eid_len_offset);
	}
#endif
