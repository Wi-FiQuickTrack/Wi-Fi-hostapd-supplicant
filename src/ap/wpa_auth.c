...
#ifdef CONFIG_WFA
	if (msg == PAIRWISE_4) {
		wpa_msg(wpa_auth->conf.msg_ctx, MSG_INFO, "M4_KEY_INFO %d", key_info);
	} else if (msg == PAIRWISE_2) {
		wpa_msg(wpa_auth->conf.msg_ctx, MSG_INFO, "M2_KEY_INFO %d", key_info);
	}
#endif /* CONFIG_WFA */
...
#if defined(CONFIG_TESTING_OPTIONS) && defined(CONFIG_WFA)
			if (wpa_auth->conf.ignore_sta_gtk_rekey) {
				wpa_auth_logger(wpa_auth, wpa_auth_get_spa(sm),
						LOGGER_INFO,
						"ignore EAPOL-Key Request from STA for GTK rekeying");				
				goto out;
			}
#endif /* CONFIG_WFA && CONFIG_TESTING_OPTIONS*/
...
SM_STATE(WPA_PTK, PTKCALCNEGOTIATING)
{
...
#ifdef CONFIG_WFA
	size_t hex_len = 2 * key_data_length + 1;
	char *hex = os_malloc(hex_len);
	if (hex) {
		wpa_snprintf_hex(hex, hex_len, key_data, key_data_length);
		wpa_hexdump_key(MSG_DEBUG, "WPA: M2 Key Data", key_data, key_data_length);
		wpa_msg(wpa_auth->conf.msg_ctx, MSG_INFO, "M2_KDE_IE %s", hex);
		os_free(hex);
	}
#endif /* CONFIG_WFA */
	...
	if ((wpa_auth->conf.rsn_override_key_mgmt ||
		...
		wpa_msg(wpa_auth->conf.msg_ctx, MSG_INFO, "M2_VALID_SNONCE");
	}
#endif /* CONFIG_WFA */ 
...
#ifdef CONFIG_WFA
		wpa_msg(wpa_auth->conf.msg_ctx, MSG_INFO, "EAPOL-M2-ML-VALIDATE-FAILURE "
			MACSTR, MAC2STR(sm->addr));
#endif
		wpa_sta_disconnect(wpa_auth, sm->addr,
				   WLAN_REASON_PREV_AUTH_NOT_VALID);
		...
		*pos = link_id;
		...
		if (rsne_len) {
			...
			if (wpa_key_mgmt_ft(sm->wpa_key_mgmt)) {
				...
				pos -= rsne_len;
				res = wpa_insert_pmkid(pos, &rsne_len, sm->pmk_r1_name, true);
				if (res < 0) {
					wpa_printf(MSG_ERROR,
...
SM_STATE(WPA_PTK, PTKINITNEGOTIATING)
{
	...
	if (gtk && !is_mld) {
		hdr[0] = gtkidx & 0x03;
#ifdef CONFIG_WFA		
		if (conf->gtk_kde_random_reserved_bits) {
			/* Reserved bits of GTK KDE randomly set to one */
			u8 dummy[2];
			if (random_get_bytes(dummy, 2) < 0)
				goto done;
			hdr[0] = (gtkidx & 0x03) | (dummy[0] & 0xf8);
			hdr[1] = dummy[1];
		}
#endif
...
#ifdef CONFIG_WFA
		wpa_msg(sm->wpa_auth->conf.msg_ctx, MSG_INFO, "EAPOL-M4-ML-VALIDATE-FAILURE "
			MACSTR, MAC2STR(sm->addr));
#endif
		wpa_sta_disconnect(sm->wpa_auth, sm->addr,
				   WLAN_REASON_PREV_AUTH_NOT_VALID);
...
SM_STATE(WPA_PTK_GROUP, REKEYESTABLISHED)
{
	...
	wpa_auth_vlogger(wpa_auth, wpa_auth_get_spa(sm), LOGGER_INFO,
			 ...
			 sm->wpa == WPA_VERSION_WPA ? "WPA" : "RSN");
#ifdef CONFIG_WFA
	wpa_msg(wpa_auth->conf.msg_ctx, MSG_INFO, "EAPOL-GROUP-HS-COMPLETED "
		MACSTR, MAC2STR(sm->addr));
#endif
	sm->has_GTK = true;
}
