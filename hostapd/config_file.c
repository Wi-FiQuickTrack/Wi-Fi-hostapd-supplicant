...
#ifdef CONFIG_WFA
static int hostapd_parse_element_extend(struct hostapd_config *conf, char *pos)
{
	int eid;
	char *end;
	...
	while (*pos != '\0') {
		...
		eid = atoi(pos);
		if (eid == WLAN_EID_MULTIPLE_BSSID) {
			conf->mbssid_extend = true;
		} else if (eid == WLAN_EID_BSS_MAX_IDLE_PERIOD) {
			conf->bss_max_idle_extend = true;
		} else if (eid == WLAN_EID_EXT_CAPAB) {
			conf->ext_capa_extend = true;
		} else if (eid == WLAN_EID_WIDE_BW_CHSWITCH) {
			conf->wide_band_cs_extend = true;
		} else if (eid == WLAN_EID_TRANSMIT_POWER_ENVELOPE) {
			conf->tpe_extend = true;
		} else if (eid == WLAN_EID_REDUCED_NEIGHBOR_REPORT) {
			conf->rnr_extend = true;
		} else if (eid == WLAN_EID_EXTENSION) {
			char *ext_ptr = end;
			end = os_strchr(ext_ptr + 1, ' ');
			int ext_id = atoi(ext_ptr + 1);
			if (ext_id == WLAN_EID_EXT_HE_CAPABILITIES) {
				conf->he_capa_extend = true;
			} else if (ext_id == WLAN_EID_EXT_HE_MU_EDCA_PARAMS) {
				conf->mu_edca_extend = true;
			} else if (ext_id == WLAN_EID_EXT_HE_6GHZ_BAND_CAP) {
				conf->he_6g_capa_extend = true;
			} else if (ext_id == WLAN_EID_EXT_EHT_OPERATION) {
				conf->eht_oper_extend = true;
			} else if (ext_id == WLAN_EID_EXT_EHT_CAPABILITIES) {
				conf->eht_capa_extend = true;
			} else if (ext_id == WLAN_EID_EXT_MULTI_LINK) {
				conf->ml_extend = true;
			} else {
				wpa_printf(MSG_ERROR, "Unknown ext id %d for element extend", ext_id);
				return -1;
			}
		} else {
			wpa_printf(MSG_ERROR, "Unknown id %d for element extend", eid);
			return -1;
		}
		if (!end)
			break;
...
static int parse_element_reserved_bits(struct hostapd_config *conf, char *pos)
{
	char *token, *saveptr;
	bool capa_info_set = false;
	...
	while (token) {
		...
		if (end) {
			*end = '\0';
			...
			if (!os_strncmp(sub_pos, "capa_info", 9))
				capa_info_set = true;
			else
				eid = atoi(sub_pos);
		}
		...
		if (capa_info_set) {
			conf->capa_info_res_bits = true;
			capa_info_set = false;
		} else if (eid == WLAN_EID_RRM_ENABLED_CAPABILITIES) {
			conf->rrm_res_bits = true;
		} else if (eid == WLAN_EID_BSS_MAX_IDLE_PERIOD) {
			conf->bss_max_idle_res_bits = true;
		} else if (eid == WLAN_EID_REDUCED_NEIGHBOR_REPORT) {
			conf->rnr_res_bits = true;
		} else if (eid == WLAN_EID_EXTENSION) {
			if (ext_id == WLAN_EID_EXT_HE_CAPABILITIES) {
				conf->he_capa_res_bits = true;
			} else if (ext_id == WLAN_EID_EXT_HE_OPERATION) {
				conf->he_oper_res_bits = true;
			} else if (ext_id == WLAN_EID_EXT_HE_6GHZ_BAND_CAP) {
				conf->he_6g_capa_res_bits = true;
			} else if (ext_id == WLAN_EID_EXT_EHT_OPERATION) {
				conf->eht_oper_res_bits = true;
			} else if (ext_id == WLAN_EID_EXT_MULTI_LINK) {
				conf->ml_res_bits = true;
			} else if (ext_id == WLAN_EID_EXT_EHT_CAPABILITIES) {
				conf->eht_capa_res_bits = true;
			} else {
				wpa_printf(MSG_ERROR, "Unknown EXT_ID for reserved bits: %d", ext_id);
				return -1;
			}
		} else {
			wpa_printf(MSG_ERROR, "Unknown EID for reserved bits: %d", eid);
			return -1;
		}
...
static int parse_element_untested_bits(struct hostapd_config *conf, char *pos)
{
    char *token, *saveptr;
    ...
    while (token) {
        ...
        if (eid == WLAN_EID_EXTENSION) {
            if (ext_id == WLAN_EID_EXT_EHT_OPERATION) {
                conf->eht_oper_unt_bits = true;
            } else if (ext_id == WLAN_EID_EXT_EHT_CAPABILITIES) {
                conf->eht_capa_unt_bits = true;
            } else {
                wpa_printf(MSG_ERROR, "Unknown ext_id for untested bits: %d", ext_id);
                return -1;
            }
        } else {
            wpa_printf(MSG_ERROR, "Unknown eid for untested bits: %d", eid);
            return -1;
        }
...
#ifdef CONFIG_WFA
	} else if (os_strcmp(buf, "rx_ss_support") == 0) {
		conf->rx_ss_support = atoi(pos);
#endif
...
#define PARSE_TEST_PROBABILITY(_val)				\
	...
	} else if (os_strcmp(buf, "skip_6g_bss_security_check") == 0) {
		bss->skip_6g_bss_security_check = atoi(pos);
#ifdef CONFIG_WFA
	} else if (os_strcmp(buf, "ignore_sta_gtk_rekey") == 0) {
		bss->ignore_sta_gtk_rekey = atoi(pos);
	} else if (os_strcmp(buf, "element_extend") == 0) {
		if (hostapd_parse_element_extend(conf, pos))
			return 1;
	} else if (os_strcmp(buf, "element_reserved_bits") == 0) {
        if (parse_element_reserved_bits(conf, pos))
            return 1;
	} else if (os_strcmp(buf, "element_untested_bits") == 0) {
        if (parse_element_untested_bits(conf, pos))
            return 1;
	} else if (os_strcmp(buf, "ml_ctrl_ext_mld_capa_pres_bit") == 0) {
        if (atoi(pos))
            conf->ml_ctrl_ext_mld_capa_pres_bit = true;
#endif /* CONFIG_WFA */
	...
	} else if (os_strcmp(buf, "gtk_kde_random_reserved_bits") == 0) {
		bss->gtk_kde_random_reserved_bits = atoi(pos);
